...
implements _ViewModel {
 ...
 late BookDataEntity bookData;
  ...
  void _onDownloadFailed(dynamic x){
    if(!_pageStateStreamController.isClosed) {
      _pageStateSink?.add(ErrorState("Download failed!","Something error occurred while downloading your requested file."));
    }
  }
  ...
  Future<void> saveFileToAndroidStorage({required File file,})async{
    try{
      ...
      bool? isGranted = await _saf.getDirectoryPermission(grantWritePermission: true, isDynamic: true);
      ...
      persistedDirs?.forEach((path) async {
        ...
        await getExternalStorageDirectory().then((d) {
          if (d != null && d.path.toLowerCase().contains("/storage/emulated/")) {
  ...
  Future<void> saveFileToiOSStorage({required File file,})async{
